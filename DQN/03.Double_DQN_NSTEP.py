...
from agents.Double_DQN_NSTEP import Model
...
for frame_idx in range(1, max_frames + 1):
    ...
    if done:
        ...
        if np.mean(model.rewards[-10:]) > 20:
            plot(frame_idx, model.rewards, model.losses, model.sigma_parameter_mag,
                 timedelta(seconds=int(timer() - start)),name="DDQN")
            save_plot(frame_idx, model.rewards, model.losses, model.sigma_parameter_mag,
                      timedelta(seconds=int(timer() - start)),
                      nstep=model.nsteps, name="DDQN")
            print("达到20提前结束，结束轮次,", frame_idx)
            break

    # 每一万次迭代绘制训练信息
    if frame_idx % config.polt_num == 0:
        plot(frame_idx, model.rewards, model.losses, model.sigma_parameter_mag, timedelta(seconds=int(timer() - start)),model.nsteps,name='DDQN')
    ...
    if frame_idx % process_count == 0:
        ...
        process_time = timer()
model.save_weight(model_name="DoubleDQN_"+str(model.nsteps)+"STEP")
env.close()
print("输出完毕")
