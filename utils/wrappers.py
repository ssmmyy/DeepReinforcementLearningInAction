...
class ImageToPyTorch(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.observation_space = Box(low=0.0, high=1.0, shape=(old_shape[-1], old_shape[0], old_shape[1]),
                                     dtype=np.uint8)

    def observation(self, observation):
        # 转换坐标轴2和0并将RGB转换成0-1浮点数
...
class WrapPyTorch(gym.ObservationWrapper):
    def __init__(self, env=None):
        ...
        self.observation_space = Box(self.observation_space.low[0, 0, 0], self.observation_space.high[0, 0, 0],
                                     [obs_shape[2], obs_shape[1], obs_shape[0]], dtype=self.observation_space.dtype)
